...
NOVA_VERSION=`sudo nova-manage version | awk -F. '{print $1}'`
...
export CONTEGO_VERSION=`curl -s http://${TVAULT_APPLIANCE_IP}:8081/packages/ | grep tvault-contego-[0-9] | cut -c56-60`	
   ...
   if [ "$CONTEGO_VERSION" != "$CONTEGO_VERSION_INSTALLED" ]; then
        ...
        echo -e "Installed latest datamover package sucessfully\n"
   fi
